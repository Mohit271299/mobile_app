...
import 'package:magnet_update/model_edit_class/remove_credit.dart';
...
class creditEditScreen extends StatefulWidget {
  final int? credti_edit_id;

  const creditEditScreen({Key? key, this.credti_edit_id}) : super(key: key);
...
class _creditEditScreenState extends State<creditEditScreen> {
  ...
  final Credit_note_Edit_controller _creditOrderEditController = Get.put(
      Credit_note_Edit_controller(),
      tag: Credit_note_Edit_controller().toString());
  ...
  Future<void> add_product() async {
    await showModalBottomSheet(
      ...
      model.add(edit_credit_order_model(
      ...
      model_texable.add(credit_texable_value(
  ...
  List<edit_credit_order_model> model = [];

  List<credit_texable_value> model_texable = [];
  ...
  Future Credit_Edit_get() async {
    Token = await PreferenceManager().getPref(Api_url.token);
    var url =
        'https://magnetbackend.fsp.media/api/credit-note/${widget.credti_edit_id}}';
    ...
    if (response.statusCode == 200) {
      ...
      setState(() {
        ...
        for (int i = 0; i < productItems.length; i++) {
          ...
          product_name_list.add("${productItems[i]["product"]["product_name"]}");
        ...
        dateController.text = data["credit_note_date"];
        CreditNoteNumberController.text = data["credit_note_number"];
        ...
        (data["invoice_type"] == 'tax-invoice'
            ...
            : (data["invoice_type"] == 'bill-of-supply'
                ? selectedinvoice = 'bill-of-supply'
                : (data["invoice_type"] == 'cash-memo'
                    ? selectedinvoice = 'cash-memo'
                    : selectedinvoice = 'tax-invoice')));

        (selectedinvoice == 'tax-invoice' || selectedinvoice == 'bill-of-supply'
        ...
        (data["invoice_type"] == 'tax-invoice'
            ...
            : (data["invoice_type"] == 'bill-of-supply'
                ? bill_no = invoice["bill_of_supply_number"]
                : (data["invoice_type"] == 'cash-memo'
                    ? cash_no = invoice["sr_number"]
                    : invoice_no = invoice["invoice_number"])));
       (data["invoice_type"] == 'tax-invoice'
    ...
    return Scaffold(
      ...
      body: custom_header(
        ...
        data:InkWell(
          ...
          child: GetBuilder<Credit_note_Edit_controller>(
              init: _creditOrderEditController,
              builder: (_) {
                return PageView(
  ...
  Future credit_note_API() async {
    ...
    Map data = {
      ...
      "invoice_id": (selectedinvoice == 'tax-invoice'
          ...
          : (selectedinvoice == 'bill-of-supply'
              ? Bill_id
              : (selectedinvoice == 'cash-memo' ? Cash_id : selectedinvoice == 'tax-invoice'))),
      ...
      "entity_type": (selectedinvoice == 'tax-invoice'
          ...
          : (selectedinvoice == 'bill-of-supply'
              ? "customer"
              : (selectedinvoice == 'cash-memo' ? "other" : "customer"))),
    ...
    var url = Api_url.credit_note_add_api + '/${widget.credti_edit_id}';
  ...
  Future getBill_supplyList() async {
    ...
    if (response.statusCode == 200) {
      Map<String, dynamic> Books = json.decode(response.body);
      billSupplyList = Books["data"];
      print("billSupply_222e");
      print(billSupplyList);
      // print("textinvoiceNumberList");
      // print(textinvoiceNumberList);
    } else {
      print(response.statusCode);
  ...
  Future default_cash_memo() async {
    ...
    if (response.statusCode == 200) {
      final Map<String, dynamic> responseBody =
          json.decode(response.body.replaceAll('}[]', '}'));
      setState(() {
        cash_memo_id = responseBody["data"]["id"];
        cash_memo_name = responseBody["data"]["account_name"];
        print(cash_memo_id);
