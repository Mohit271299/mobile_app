...
import 'package:magnet_update/model_edit_class/remove_proforma.dart';
...
class proformaEditScreen extends StatefulWidget {
  final int? proforma_id;
  const proformaEditScreen({Key? key, this.proforma_id}) : super(key: key);

  @override
  _proformaEditScreenState createState() => _proformaEditScreenState();
}

class _proformaEditScreenState extends State<proformaEditScreen>  {
  GlobalKey<ScaffoldState>? _globalKey = GlobalKey<ScaffoldState>();
  final ProformaInvoice_Edit_controller _proformaInvoiceEditController= Get.put(
      ProformaInvoice_Edit_controller(),
      tag: ProformaInvoice_Edit_controller().toString());
  ...
  final List<String> typea = <String>[
    'Exclusive of Tax',
    'Inclusive of Tax',
    'Out of Scope of Tax'
  ];
  ...
  init() async{
    ...
    await  proforma_invoice_Edit_get();
  ...
  List<edit_proforma_invoice_model> model = [];

  List<texable_proforma_value> model_texable = [];
  ...
  Future proforma_invoice_Edit_get() async {
    Token = await PreferenceManager().getPref(Api_url.token);
    var url =
        'https://magnetbackend.fsp.media/api/proforma-invoice/${widget.proforma_id}}';
    ...
    if (response.statusCode == 200) {
      ...
      setState(() {
        ...
        for (int i = 0; i < productItems.length; i++) {
          ...
          model.add(edit_proforma_invoice_model(
          ...
          model_texable.add(texable_proforma_value(
        ...
        // so_dateController.text = data["so_date"];
        //
        // so_numberController.text = data["so_number"];
        dc_numberController.text = data["dc_number"];
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: custom_header(
        ...
        data: GetBuilder<ProformaInvoice_Edit_controller>(
            init: _proformaInvoiceEditController,
            builder: (_) {
              return PageView(
                                ...
                                children: [
                                  ...
                                  Row(
                                    ...
                                    children: [
                                      GestureDetector(
                                        onTap: () {
                                          _proformaInvoiceEditController
                                              .pageIndexUpdate('02');
  ...
  String Token = "";
  int Counter = 0 ;
  Future proforma_invoice_API() async {
    Token = await PreferenceManager().getPref(Api_url.token);
    print('calling');
    ...
    var url = Api_url.proforma_invoice_add_api;
