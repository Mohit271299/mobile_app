...
import 'package:magnet_update/model_edit_class/remove_estimate.dart';
...
class estimateEditScreen extends StatefulWidget {
  ...
  const estimateEditScreen({Key? key, this.estimate_id}) : super(key: key);
...
class _estimateEditScreenState extends State<estimateEditScreen>  {
  ...
  final Estimate_Edit_controller _estimateEditController =Get.put(
      Estimate_Edit_controller(),
      tag: Estimate_Edit_controller().toString());
  ...
  TextEditingController EstimatenumberController = new TextEditingController();
  ...
  Future<void> add_product() async {
    ...
    await showModalBottomSheet(
      ...
      model_texable.add(texable_estimate_value(
  ...
  Future<void> _showIOS_DatePicker(ctx) async {
    ...
    if (pickedDate != null) {
      ...
      setState(() {
        dateController.text = DateFormat('dd-MM-yyyy').format(pickedDate!); //set output date to TextField value.
      });
    } else {
  ...
  init() async{
    ...
    await  estimate_invoice_Edit_get();
  ...
  List<edit_estimate_model> model = [];

  List<texable_estimate_value> model_texable = [];
  ...
  Future estimate_invoice_Edit_get() async {
    Token = await PreferenceManager().getPref(Api_url.token);
    var url =
        'https://magnetbackend.fsp.media/api/estimate-invoice/${widget.estimate_id}}';
    ...
    if (response.statusCode == 200) {
      ...
      setState(() {
        ...
        EstimatenumberController.text = data["invoice_number"];
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: custom_header(
        ...
        data: GetBuilder<Estimate_Edit_controller>(
            init: _estimateEditController,
            builder: (_) {
              return Container(
  ...
  Future Estimate_API() async {
    ...
    Map data = {
      // "id": 0,
      "invoice_number": EstimatenumberController.text,
    ...
    var url = Api_url.estimate_add_api + '/${widget.estimate_id}';
