...
import 'package:magnet_update/model_edit_class/remove_debit.dart';
...
class debitEditScreen extends StatefulWidget {
  final int? debit_edit_id;
  const debitEditScreen({Key? key, this.debit_edit_id}) : super(key: key);

  @override
  _debitEditScreenState createState() => _debitEditScreenState();
}

class _debitEditScreenState extends State<debitEditScreen> {
  GlobalKey<ScaffoldState>? _globalKey = GlobalKey<ScaffoldState>();
  final Debit_note_Edit_controller _debitOrderEditController  = Get.put(
      Debit_note_Edit_controller(),
      tag: Debit_note_Edit_controller().toString());
  ...
  TextEditingController DebitNoteNumberController =
                      ...
                      itemBuilder: (BuildContext context, int index) {
                        return GestureDetector(
                          onTap: () {
                            ...
                            setState(() {
                              vendor_name = VendorsData[index].accountName;
  ...
  void select_vendor_invoice_number() {
    showModalBottomSheet(
        context: context,
        isScrollControlled: true,
        builder: (context) {
                ...
                children: [
                  ...
                  Expanded(
                    child: ListView.builder(
                      ...
                      shrinkWrap: true,
                      scrollDirection: Axis.vertical,
                      itemCount: purchaseInvoiceList_data.length,
                      itemBuilder: (BuildContext context, int index) {
                        return GestureDetector(
                          onTap: () {
                            //  selected_customer= true;
                            ...
                            setState(() {
                              vendro_invoice_no = purchaseInvoiceList_data[index]["invoice_number"];
                              vendor_invoice_id = purchaseInvoiceList_data[index]["id"];
                            });
                            print("vendro_invoice_no");
                            print(vendro_invoice_no);
                            Navigator.pop(context);
                          },
                          child: Container(
                                      ...
                                      children: [
                                        Container(
                                          margin: EdgeInsets.only(
                                              left: 12, top: 12, right: 19),
                                          child: Text(
                                              "${purchaseInvoiceList_data[index]["invoice_number"]}",
                                              style: FontStyleUtility.h15(
                                                fontColor:
                                                ColorUtils.blackColor,
                                                fontWeight: FWT.semiBold,
                                        ...
                                        Container(
                                          margin: EdgeInsets.only(
                                              left: 12, top: 4, right: 19),
                                          child: Text(
                                              "${purchaseInvoiceList_data[index]["invoice_date"]}",
                                              style: FontStyleUtility.h12(
                                                fontColor: ColorUtils.ogfont,
                                                fontWeight: FWT.medium,
                                              )),
                                        ...
                                        RichText(
                                          text: TextSpan(
                                            ...
                                            children: <TextSpan>[
                                              TextSpan(
                                                text:
                                                "${purchaseInvoiceList_data[index]["total_amt"]}/-",
                                                style: FontStyleUtility.h12(
                                                  fontColor:
                                                  ColorUtils.blueColor,
                                                  fontWeight: FWT.semiBold,
  ...
  Future<void> add_product() async {
    await showModalBottomSheet(
      ...
      model.add(edit_debit_order_model(
      ...
      model_texable.add(debit_texable_value(
  ...
  List<edit_debit_order_model> model = [];

  List<debit_texable_value> model_texable = [];
  ...
  Future debit_Edit_get() async {
    Token = await PreferenceManager().getPref(Api_url.token);
    var url =
        'https://magnetbackend.fsp.media/api/debit-note/${widget.debit_edit_id}}';
    ...
    if (response.statusCode == 200) {
      ...
      setState(() {
        ...
        dateController.text = data["debit_note_date"];
        DebitNoteNumberController.text = data["debit_note_number"];

        Gst_no_Controller.text = data["gst_number"];
        place_of_supply_Controller.text = data["place_of_supply"];
        selectedValue = data["tax_type"];
        invoicedateController.text = invoice["invoice_date"];
        stateController.text = data["place_of_supply"];
        vendro_invoice_no = invoice["invoice_number"];
        vendor_invoice_id = invoice["id"];

        vendor_billing_name = vendor["billing_name"];
        vendor_id = vendor["id"];
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: custom_header(
        ...
        data:GetBuilder<Debit_note_Edit_controller>(
            init: _debitOrderEditController,
            builder: (_) {
              return Container(
  ...
  Future debit_note_API() async {
    ...
    Map data = {
      ...
      // credit_note_number: ""
      "debit_note_number": DebitNoteNumberController.text,
      "debit_note_date": dateController.text,
    ...
    var url = Api_url.debit_note_add_api + '/${widget.debit_edit_id}';
  ...
  Future purchase_invoiceList() async {
    Token = await PreferenceManager().getPref(Api_url.token);
    var url =
        'https://magnetbackend.fsp.media/api/purchase?recordsPerPage=10&pageNumber=1&sortOrder=ASC&sortBy=invoice_date&vendorId=$vendor_id';
    ...
    if (response.statusCode == 200) {
      ...
      purchaseInvoiceList_data = Books["data"];
      print("PurchaseList");
      print(purchaseInvoiceList_data);
