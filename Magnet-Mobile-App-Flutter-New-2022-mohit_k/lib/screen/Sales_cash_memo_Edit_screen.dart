...
import 'package:magnet_update/model_edit_class/remove_cash_memo.dart';
...
class cash_memoEditScreen extends StatefulWidget {
  final int? cash_memo_id;
  const cash_memoEditScreen({Key? key, this.cash_memo_id}) : super(key: key);

  @override
  _cash_memoEditScreenState createState() => _cash_memoEditScreenState();
}

class _cash_memoEditScreenState extends State<cash_memoEditScreen> {
  final GlobalKey<ScaffoldState>? _globalKey = GlobalKey<ScaffoldState>();
  final CashMemo_Edit_controller _cashMemoEditController = Get.put(
      CashMemo_Edit_controller(),
      tag: CashMemo_Edit_controller().toString());
                                        ...
                                        RichText(
                                          text: TextSpan(
                                            ...
                                            children: <TextSpan>[
                                              TextSpan(
                                                text:
                                                "${OthersData[index].openingBalance}/-",
                                                style: FontStyleUtility.h12(
                                                  fontColor: ColorUtils.blueColor,
                                                  fontWeight: FWT.semiBold,
                                                ),
  ...
  await showModalBottomSheet(
      ...
      model.add(edit_cash_memo_model(
      ...
      model_texable.add(cash_memo_texable_value(
  ...
  List<edit_cash_memo_model> model = [];

  List<cash_memo_texable_value> model_texable = [];
  ...
  Future cash_memo_Edit_get() async {
    Token = await PreferenceManager().getPref(Api_url.token);
    var url =
        'https://magnetbackend.fsp.media/api/cashmemo/${widget.cash_memo_id}';
    ...
    if (response.statusCode == 200) {
      ...
      setState(() {
        ...
        for (int i = 0; i < productItems.length; i++) {
          ...
          double disc = (amount *
              double.parse(
                  model[i].product_dis.toString())) /
              100;
          double ammountInti = amount - disc;
          double taxExl = ammountInti *
          ...
          product_texable_inl_price_list.add(double.parse(total_incl_price_string));
          ...
          product_total_exl_price_list.add(double.parse(total_excl_price_string));
        ...
        Sr_NumberController.text = data["sr_number"];
        ...
        others_name = others["billing_name"];
        others_company_name = others["account_name"];
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: custom_header(
        ...
        data: GetBuilder<CashMemo_Edit_controller>(
            init: _cashMemoEditController,
            builder: (_) {
              return Stack(
                children: [
                                                              ...
                                                              children: [
                                                                ...
                                                                GestureDetector(
                                                                  onTap: (){
                                                                    setState(() {
                                                                      print(index);
                                                                      model.indexOf(model[index]);
                                                                      model.removeAt(index);
                                                                      model_texable.removeAt(index);
                                                                      // tax_invoice_Edit_get();
                                                                      // model_texable.re
                                                                    });
                                                                    print(model);
  ...
  Future cash_memo_API() async {
    ...
    var url = Api_url.cash_memo_add_api+ '/${widget.cash_memo_id}';
